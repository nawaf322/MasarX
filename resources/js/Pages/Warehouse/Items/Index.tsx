...
export default function WarehouseItemsIndex({ items, filters }: Props) {
    const { t } = useTranslation();
    const [search, setSearch] = useState(filters.search ?? '');

    function handleSearch(e: React.FormEvent) {
        e.preventDefault();
        router.get(route('warehouse.inventory.items.index'), { search }, { preserveState: true, replace: true });
    }

    return (
        <AuthenticatedLayout header={
            <div className="flex items-center justify-between">
                <h2 className="text-xl font-semibold">{t('warehouse.items_title')}</h2>
                <Link href={route('warehouse.index')} className="text-sm text-blue-600 hover:underline">
                    ← {t('warehouse.back_to_warehouse')}
                </Link>
            </div>
        }>
            <Head title={t('warehouse.items_title')} />
...
function ItemCard({ item, t }: { item: InventoryItem; t: (k: string) => string }) {
    ...
    function handleFiles(files: FileList | null) {
        ...
        fetch(route('warehouse.inventory.items.photos', item.id), {
            method: 'POST',
        ...
        <div className="bg-white rounded-lg shadow border border-gray-200 overflow-hidden">
            ...
            <div className="p-3">
                <div className="flex items-center justify-between mb-1">
                    <span className="text-xs font-mono text-gray-500 bg-gray-100 px-1.5 py-0.5 rounded">{item.sku}</span>
                ...
                <p className="text-xs text-gray-400 mt-1">{t('warehouse.unit')}: {item.unit}</p>
            </div>
