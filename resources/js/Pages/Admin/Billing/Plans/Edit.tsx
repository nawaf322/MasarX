...
import {
    ...
    Plus, X, CheckCircle2, XCircle, TrendingDown, Loader2,
} from 'lucide-react';
...
interface Plan {
    ...
    price_semiannual?: number | string | null;
...
function limitsToRows(limits?: Record<string, string | number>): { key: string; value: string }[] {
    if (!limits || typeof limits !== 'object') return [];
    return Object.entries(limits).map(([k, v]) => ({ key: k, value: String(v) }));
}
...
// Reusable price input with left addon showing the currency symbol
function PriceInput({
...
export default function EditPlan({ plan }: { plan: Plan }) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        price_monthly:     String(Number(plan.price_monthly) || 0),
        price_quarterly:   plan.price_quarterly  != null ? String(Number(plan.price_quarterly))  : '',
        price_semiannual:  plan.price_semiannual != null ? String(Number(plan.price_semiannual)) : '',
        price_annual:      plan.price_annual     != null ? String(Number(plan.price_annual))     : '',
        ...
        limits:            rowsToLimits(limitsToRows(plan.limits)),
    });
    ...
    const submit = (e: React.FormEvent) => { e.preventDefault(); put(route('admin.billing.plans.update', plan.id)); };

    const commonLimitKeys = ['max_users', 'max_shipments', 'max_branches', 'max_api_calls', 'storage_gb'];

    return (
        <AuthenticatedLayout>
            <Head title={`${t('common.edit')} — ${plan.name}`} />
                ...
                <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                            <h1 className="text-2xl font-bold text-foreground truncate">{plan.name}</h1>
                            <Badge className={data.is_active
                                ...
                                {data.is_active
                                    ? <><CheckCircle2 className="h-3 w-3 mr-1 inline" />{t('common.active')}</>
                                    : <><XCircle className="h-3 w-3 mr-1 inline" />{t('common.inactive')}</>
                                }
                            </Badge>
                        </div>
                                ...
                                <CardContent className="space-y-3">
                                    ...
                                    <div className="flex items-center gap-2 pt-1 border-t border-border/50">
                                        <div className="relative flex-1">
                                            <Input list="limit-keys-edit" className="font-mono text-sm"
                                                placeholder="max_users" value={newLimitKey}
                                                onChange={e => setNewLimitKey(e.target.value)} />
                                            <datalist id="limit-keys-edit">
