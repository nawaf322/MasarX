...
class StoreUserRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            // Optional manual password — if omitted, invitation email flow is used instead
            'password' => ['nullable', 'confirmed', Rules\Password::min(8)->mixedCase()->numbers()->symbols()],
            'role' => 'required|string|exists:roles,name',
            'branch_id' => 'nullable|integer|exists:branches,id',
            'department_id' => array_filter(['nullable', 'integer', \Illuminate\Support\Facades\Schema::hasTable('departments') ? 'exists:departments,id' : null]),
            'document_id' => [
                'nullable',
                'string',
                'max:50',
                Rule::when($this->filled('document_id'), [
                    Rule::unique('users', 'document_id')->where('organization_id', $orgId),
                ]),
            ],
        ];
    }
